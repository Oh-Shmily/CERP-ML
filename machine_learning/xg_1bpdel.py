...
model_save_path = '/machine_learning/model_param/xg_1bp_del.pkl'
...
def train_valid(train_path, valid_path, model, model_save_path):
    ...
    y_train = np.sum(train_df[['-1+1', '0+1', '1+1']], axis=1)
    X_valid = dna_to_onehot(valid_df['target_seq'])
    y_valid = np.sum(valid_df[['-1+1', '0+1', '1+1']], axis=1)
...
def test(test_path, model_save_path, seed):
    ...
    y_test = np.sum(forecast_test_df[['-1+1', '0+1', '1+1']], axis=1)
    X_lindel = dna_to_onehot(lindel_test_df['target_seq'])
    y_lindel = np.sum(lindel_test_df[['-1+1', '0+1', '1+1']], axis=1)
    X_sprout = dna_to_onehot(sprout_test_df['target_seq'])
    y_sprout = sprout_test_df['prob_1bpdel']
